...
- `nex-n2.5-pro` sometimes answers with a JSON object instead of prose (9 of 187 replies in later testing, mostly aquarium questions), and the key varies (`message`, `response`, `answer`, `question`, `assistant_response`, `assistantMessage`, or a structured `{"water_type": ..., "parameters": [...]}`) and is often cut off part-way (missing braces, or truncated inside nested content). `api/chat.js` never lets raw JSON reach a customer (anything that starts like a JSON object, even in a ```` ```json ```` fence, is repaired or becomes an error; a test cuts sample replies at every possible length to check this): an object with a prose field shows just that text, a flat structured object is shown as a bullet list, and JSON it cannot render becomes a `bad_response` error (the customer sees the friendly error copy). Replies that are not a JSON object are untouched.
